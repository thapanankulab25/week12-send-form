    ...
    <div className="mt-3 mb-5 mx-auto p-3 rounded"
      ...
      <form onSubmit={handleSubmit(onSubmitForm)}>
        <div className="form-group mb-3">
          ...
          <select id="cats" className="form-select form-select-sm" {...register('category')}>
            {cats.map(item => {
        ...
        <div className="form-group mb-3">
          ...
          {errors.detail && <div style={err}>ต้องกำหนดรายละเอียดสินค้า</div>}        </div>
        <div className="form-group mb-3">
          <label htmlFor="dateAdded">วันที่เพิ่มสินค้า *</label>
          <input type="date" id="dateAdded" name="dateAdded"
            className="form-control form-control-sm" {...register('dateAdded', { required: true })}/>
        </div>
        <div className="form-group mb-3">
          ...
          {opts.map((opt, i) => (
            <div className="form-check form-check-inline mb-2" key={i}>
              <input type="checkbox" id={'opt' + i} value={opt}
                className="form-check-input" {...register(`options.${i}`)} />
              <label htmlFor={'opt' + i} className="form-check-label">{opt}</label>
            </div>
          ))}
        </div>
        <div className="text-center mt-4">
          <button className="btn btn-primary btn-sm px-4" type="submit">OK</button>
        </div>
      </form>
    </div>
  );
